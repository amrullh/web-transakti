...
export default function ModalTambahProduk({ onClose, onSave }: { onClose: () => void; onSave: (data: Omit<ProductType, "id" | "status">) => void; }) {
  ...
  return (
    <div className={modal.overlay}>
      <div className={modal.modalLarge}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 180px", gap: 28, alignItems: "start" }}>
          <div>
            ...
            <select style={{ width: "106%", padding: 10, marginBottom: 12 }} value={form.kategori} onChange={(e) => setForm({ ...form, kategori: e.target.value as ProductType["kategori"] })}>
        ...
        <div className={modal.modalButtons}>
          ...
          <button className={modal.btnPrimary} onClick={() => { onSave({ nama: form.nama, kode_produk: form.kode_produk || null, harga_modal: form.harga_modal, harga_jual: form.harga_jual, stok: form.stok, kategori: form.kategori, gambar: form.gambar || null }); }}>Simpan</button>
        </div>
      </div>
    </div>
  );
